...
class Login extends DBConnection {
    ...
    function login_warden(){
        extract($_POST);
        $stmt = $this->conn->prepare("SELECT * from wardens where email = ? and `password` = ? and delete_flag = 0 ");
        $password = md5($password);
        $stmt->bind_param('ss',$email,$password);
        $stmt->execute();
        $result = $stmt->get_result();
...
switch ($action) {
	...
	case 'logout_agent':
		...
		break;
    case 'logout_warden':
        echo $auth->logout_wander();
        break;
    case 'login_warden':
        echo $auth ->login_warden();
        break;
	default:
		echo $auth->index();
		break;
